...
- Ошибка `testops_find_testcases` (невалидный AQL, 4xx/5xx, таймаут) → строка `Не проверено: покрытие TestOps — <причина>`, секцию покрытия не заполняй. «Покрытия не найдено» пиши только когда поиск прошёл без ошибки и вернул пусто — иначе главная сессия создаст дубли.
