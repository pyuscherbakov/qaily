...
2. **Найди существующее покрытие** — `testops_find_testcases` в целевом проекте (по умолчанию 35):
   - Источник — задача Redmine → сначала `aql: "tag = \"redmine-<id>\""`: кейсы, созданные тест-дизайном по этой задаче раньше. Найденное — прямое покрытие. Номера задачи Redmine нет → шаг пропусти.
   - Затем AQL по 2–4 ключевым терминам требования: `name ~= "термин1" or name ~= "термин2"`.
...
- ...
(поиск прошёл и пусто — «покрытия не найдено»)
