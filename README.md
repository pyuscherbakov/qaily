...
QA AI-ассистент для команды на базе Claude Code: плагин со скиллами тест-дизайна и ревью тест-кейсов, MCP-интеграциями (Allure TestOps, Redmine, Kaiten — карточки и документы/wiki) плюс браузерная автоматизация (Playwright, Chrome DevTools) для прогона и отладки UI-сценариев.
