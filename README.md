...
QA AI-ассистент для команды на базе Claude Code: плагин со скиллами тест-дизайна (по требованию и по покрытию API), ревью тест-кейсов, ревью автотестов и разбор упавших прогонов, MCP-интеграциями (Allure TestOps, Redmine, Kaiten — карточки и документы/wiki, Jam — записи багов с логами и сетью) плюс браузерная автоматизация (Playwright, Chrome DevTools) для прогона и отладки UI-сценариев.
...
Проект TestOps берётся из ссылки на кейс (`.../project/<projectId>/test-cases/<id>`); голый номер кейса, задача или MR — скилл спросит `projectId`. То же в ревью тест-кейса.

Пачечный режим находит кейсы задачи по тегу `redmine-<id>` (его ставит тест-дизайн); кейсы, привязанные к задаче только полем «Связанные задачи», пока не находятся — сводка об этом предупреждает.

Во всех трёх режимах кейс ревьюит суб-агент `autotest-reviewer` (`agents/autotest-reviewer.md`, read-only): алгоритм ревью одного кейса и блоки критериев описаны там, скилл занимается только режимами, сбором входных данных и сводкой. Агент читает свой промпт сам — скилл передаёт ему данные (`case_id`, `projectId`, требования, cwd, формат ответа), а не шаги.
...
`projectId` обязателен: нет в запросе — скилл спросит (автотесты — обычно проект 37, ручные прогоны — 1).

## Тест-дизайн от покрытия API (скилл coverage-gap-design)

Скилл берёт OpenAPI-спеку (файл или URL) и, если есть, JSON-отчёт swagger-coverage, находит непокрытые операции, статус-коды и параметры и генерирует кейсы только на пробелы. Кейсы помечаются тегами `api-<method>-<путь>` (`POST /cohorts` → `api-post-cohorts`) — по ним следующий запуск видит покрытие. Запись в TestOps — после подтверждения таблицы, как в `test-design`; бизнес-сценарии остаются на `test-design`.

| Вход | Пример запроса |
|------|-----------------|
| Файл спеки | «покрой API по openapi.yaml» |
| Спека + coverage | «догенерь кейсы по swagger-coverage-results.json и спеке <url>» |
