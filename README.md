...
## Подбор регрессионного тестирования (скилл regression-scope)

Скилл подбирает тест-кейсы для регресса к задаче разработки: анализирует требования Redmine-задачи и её техническое описание, определяет затронутые области функциональности и собирает тест-план в Allure TestOps из релевантных кейсов существующего покрытия.

Входные данные: номер задачи Redmine (обязателен). Целевой проект Allure — 35 по умолчанию.

Выходные данные: тест-план в Allure TestOps с ID и названием, содержащим номер Redmine-задачи; число включённых и отсеянных кейсов. В read-only режиме Allure или при отсутствии релевантных кейсов тест-план не создаётся — итоговый список выдаётся текстом.

Ограничения: Redmine и Kaiten — только чтение. Единственная операция записи — создание тест-плана (`create_test_plan`), выполняется только после явного подтверждения пользователем итогового списка кейсов. Если техническое описание задачи не найдено — скилл останавливается и спрашивает пользователя.
